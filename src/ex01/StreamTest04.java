...
public class StreamTest04 {
	...
	public static void main(String[] args) {
		...
		List<Integer> mappedList  = list.stream().map( n -> n*10 ).toList();
		System.out.println("배열 10배: " + mappedList);
		
		// 6) filter + map + sum
		double result = list.stream().filter( n -> n>=3 ).map( n -> n*n )
			.mapToDouble( Integer::doubleValue )   // sum() 할려고 Integer -> int 로 변환
			.average()
			.orElse(0.0); 
			// Optional<Double>  : 결과가 null 될수 없음
			// 만약 null 이면   .orElse(0.0) 으로 만들어 주어야 한다
			//.sum();    // 결과는 int
...
//		Stream<String>  stream  = Stream.of("apple","banana","cherry","durian");
		...
		fruitsList.stream()
			...
			.distinct()    // 중복 제거
			.sorted()	   // 정렬 해서
			.forEach( fruit -> System.out.print( fruit.toUpperCase()+" ") );
